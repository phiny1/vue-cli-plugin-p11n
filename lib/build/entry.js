...
function generateConfig (options, moduleName, version, langInfo, p11nOptions) {
  ...
  return merge(config, p11nOptions.configureRollup)
}

function getAllEntries ({ name, version }, { entry, dest }, banner, langInfo, p11nOptions) {
  const moduleName = classify(name)
  const entries = makeEntries(entry, dest, moduleName, name, banner)
  return Object.keys(entries).map(name => generateConfig(entries[name], moduleName, version, langInfo, p11nOptions))
}
